...
/* THE curtain (her 2026-07-19 pick, demo A of three): a warm sand sheet
   rises bottom -> top to cover, dwells a beat, then keeps rising away to
   reveal the new page. No mark, no second variant — the b/c demos were
   deleted once she chose. */
...
export function easedScrollTo(targetY: number, opts?: { duration?: number }) {
  ...
  // Route through Lenis when present so anchor jumps share the same momentum.
  const lenis = getLenis()
  if (lenis) {
    lenis.scrollTo(targetY, { duration: (opts?.duration ?? 1100) / 1000 })
    return
  }
...
export function TransitionProvider({ children }: { children: React.ReactNode }) {
  ...
  /* one curtain for every doorway: intercept plain internal <a>/<Link> clicks
     (gallery cards, NEXT PROJECT, footers) and route them through navigate().
     Modified clicks, downloads, external targets and same-page anchors keep
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (url.pathname === window.location.pathname) {
        if (anchor) {
          e.preventDefault()
          const el = document.getElementById(anchor)
          if (el) easedScrollTo(el.getBoundingClientRect().top + window.scrollY)
        }
        return
      }
      e.preventDefault()
      navigate(url.pathname, { anchor })
    }
    /* CAPTURE phase: Next's <Link> attaches its own click handler on the
       anchor and would route (without the curtain) before a bubble-phase
       listener ever fires; capturing first lets our preventDefault stand
       and Link bails out on defaultPrevented. */
  ...
  return (
    ...
    >
      {children}
      <TransitionOverlay phase={phase} kind={kind} navKey={navKey} />
    </TransitionContext.Provider>
  )
}
...
function TransitionOverlay({
  ...
  void kind // the old curtain/slide-up split is retired; one sand curtain now
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          transition={{
            duration: (phase === "entering" ? ENTER_MS : EXIT_MS) / 1000,
            ease: EASE,
          }}
          style={{
            position: "fixed",
            ...
            zIndex: 100,
            pointerEvents: "none",
            background: "linear-gradient(180deg, #F1E8D6 0%, #E7D8BE 55%, #D9C4A2 100%)",

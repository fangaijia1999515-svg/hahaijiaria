    ...
    <div className="border-y-section py-12 mb-16">
      ...
      >
        {metrics.map((metric, index) => {
          ...
          >
            {/* The Data - Massive display serif in the ds accent */}
            <div
              className="classic-display text-6xl md:text-7xl lg:text-8xl tracking-tight mb-2"
              style={{ color: accentColor }}
            >
              {metric.value}
            </div>

            {/* The Meaning - Mono label */}
            <div className="classic-label text-sm uppercase tracking-widest text-[var(--cl-ink)]">{metric.label}</div>

            {/* Optional Detail Caption */}
            {metric.detail && (
              <div className="text-sm text-[var(--cl-text-muted)] mt-1.5 leading-snug">
                {metric.detail}
              </div>
            )}
            </div>
